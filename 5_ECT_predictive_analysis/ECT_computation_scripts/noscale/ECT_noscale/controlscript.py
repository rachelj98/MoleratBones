...
for file in os.listdir(path):
    filename=file
    tmpscale=computeECT.computeDiameter(path+filename)
    scale=max(tmpscale,scale)
ECSlist=list()
...
outpath=cwd+'/output/'
nsublevelset=5000
xset=[3**(0.5)*scale*(2*j/float(nsublevelset-1)-1) for j in range(nsublevelset)]
print(min(xset))
print(max(xset))
print(scale)
index=0
for filename in os.listdir(path):
    ...
    for j in range(len(directions)):
        EC=list()
        for t in xset:
            EC.append(computeECT.evaluate_EC(ECSlist[index][j],t))
        for k in EC:
            file.write(str(k)+',')
        file.write('\n')
    file.close()
